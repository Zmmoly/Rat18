...
class MainActivity : AppCompatActivity() {
    ...
    private fun handleBotResponse(userMessage: String) {
        ...
        val commandSeparators = listOf("،", ",", "ثم", "\n")  // إزالة "." و "و" لأنها شائعة في الكلام العادي
        var hasMultipleCommands = false
        var potentialCommands = listOf(userMessage)
        ...
        // إذا كان هناك عدة جمل محتملة، تحقق أي منها أوامر فعلية
        if (hasMultipleCommands && potentialCommands.size > 1) {
            // فحص كل جملة - هل هي أمر؟
            ...
            // إذا وجدنا أوامر صحيحة متعددة
            if (validCommands.size > 1) {
                addBotMessage("🔄 وجدت ${validCommands.size} أوامر، سأنفذها بالترتيب...")
